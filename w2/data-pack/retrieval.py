...
def history_vector(entry: dict) -> dict:
    ...
    return {
        "id": entry.get("incident_id") or entry.get("id"),
...
def parse_action(s: str, param_names: dict) -> dict:
    parts = s.split(":")
...
def retrieve_and_vote(query: dict, history: list, actions_catalog: list, top_k: int = 5) -> dict:
