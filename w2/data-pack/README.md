...
Use Python 3.12+ in this folder and execute `python engine.py decide --incident eval/E01.json --history incidents_history.json --actions actions.yaml`. The command prints the recommended action JSON to stdout and appends one audit record to `audit.jsonl`; rerun it for E02..E08 to regenerate the full audit file.
