...
case "$FAULT" in
  latency)
    ...
    DELAY_MS="${DELAY//ms/}"
    if ! nsenter -t "$PID" -n -- tc qdisc add dev eth0 root netem delay "${DELAY_MS}ms" 2>/dev/null; then
      echo "[inject_fault] nsenter path failed; falling back to docker exec..."
